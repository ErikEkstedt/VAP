...
from typing import Any, Optional, Mapping, Iterable, Callable, Union, Iterable
...
class VAPMetricOld:
    ...
    def __init__(
        ...
        self.device = None
        self.metrics = self.initialize()

    def initialize(self):
        metrics = {"acc": {}, "f1": {}}
        for event_name in self.EVENT_NAMES:
            metrics["acc"][event_name] = Accuracy(
                task="multiclass", num_classes=2, multiclass=True, average="none"
            )
            metrics["f1"][event_name] = F1Score(
                task="multiclass",
                num_classes=2,
                multiclass=True,
                average="weighted",
    ...
    def _update_metrics(self, preds, targets):
        for event_name in self.EVENT_NAMES:
            ...
            self.metrics["acc"][event_name].update(
                preds=preds[event_name].round().cpu(), target=targets[event_name].cpu()
            )
            self.metrics["f1"][event_name].update(
                preds=preds[event_name].round().cpu(), target=targets[event_name].cpu()
            )

    def compute(self, reset: bool = False):
        score = {}
        for score_name in ["f1", "acc"]:
            score[score_name] = {}
            for event_name, metric in self.metrics[score_name].items():
                score[score_name][event_name] = metric.compute()
                if reset:
                    metric.reset()
        return score
